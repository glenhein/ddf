...
import org.codice.ddf.spatial.ogc.wfs.v1_1_0.catalog.common.Wfs11Constants.SPATIAL_OPERATORS;
import org.codice.ddf.spatial.ogc.wfs.v1_1_0.catalog.common.Wfs11JTStoGML200Converter;
...
public class WfsFilterDelegate extends SimpleFilterDelegate<FilterType> {
  ...
  private JAXBElement<PolygonType> createPolygon(String wkt) {
    ...
    //    } else {
    //      throw new IllegalArgumentException("Unable to parse Polygon coordinates from WKT
    // String");
    //    }
    return null;
  }
  ...
  private JAXBElement<LineStringType> createLineString(Geometry geometry) {
    ...
    try {
      String gml = Wfs11JTStoGML200Converter.convertGeometryToGML(geometry);
      LineStringType lineStringType =
          (LineStringType)
              Wfs11JTStoGML200Converter.convertGMLToGeometryType(gml, Wfs11Constants.LINESTRING);
      jaxbElement =
          (JAXBElement<LineStringType>)
              Wfs11JTStoGML200Converter.convertGeometryTypeToJAXB(lineStringType);
  ...
  private JAXBElement<MultiPointType> createMultiPoint(Geometry geometry) {
    ...
    try {
      ...
      MultiPointType multiPointType =
          (MultiPointType)
              Wfs11JTStoGML200Converter.convertGMLToGeometryType(gml, Wfs11Constants.POINT);
      jaxbElement =
          (JAXBElement<MultiPointType>)
              Wfs11JTStoGML200Converter.convertGeometryTypeToJAXB(multiPointType);
    } catch (JAXBException jbe) {
  ...
  private JAXBElement<MultiLineStringType> createMultiLineString(Geometry geometry) {
    ...
    try {
      ...
      MultiLineStringType multiLineStringType =
          (MultiLineStringType)
              Wfs11JTStoGML200Converter.convertGMLToGeometryType(
                  gml, Wfs11Constants.MULTI_LINESTRING);
      jaxbElement =
          (JAXBElement<MultiLineStringType>)
              Wfs11JTStoGML200Converter.convertGeometryTypeToJAXB(multiLineStringType);
    } catch (JAXBException jbe) {
  ...
  //              Wfs11JTStoGML200Converter.convertGMLToGeometryType(
  //                  gml, Wfs11Constants.GEOMETRY_COLLECTION);
  //      jaxbElement =
  //          (JAXBElement<GeometryCollectionType>)
  //              Wfs11JTStoGML200Converter.convertGeometryTypeToJAXB(geometryCollectionType);
  //    } catch (JAXBException jbe) {
  //      LOGGER.debug("Unable to create MultiPolygonType with geometry: [{}]", geometry);
  //    }
  //    return jaxbElement;
  ...
  private JAXBElement<? extends AbstractGeometryType> createGeometryOperand(String wkt) {
    ...
    if (wktGeometry instanceof Polygon) {
      ...
      if (isGeometryOperandSupported(Wfs11Constants.MULTI_POINT)) {
      ...
      if (isGeometryOperandSupported(Wfs11Constants.MULTI_LINESTRING)) {
    ...
    //      if (isGeometryOperandSupported(Wfs11Constants.GEOMETRY_COLLECTION)) {
    //        return createGeometryCollection(wktGeometry);
    //      } else {
    //        throw new IllegalArgumentException("The GeometryCollection operand is not
    // supported.");
    //      }
    //    }
