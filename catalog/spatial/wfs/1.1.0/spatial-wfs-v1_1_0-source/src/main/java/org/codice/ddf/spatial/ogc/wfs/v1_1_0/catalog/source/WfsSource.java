...
import org.codice.ddf.spatial.ogc.wfs.v1_1_0.catalog.common.DescribeFeatureTypeRequest;
import org.codice.ddf.spatial.ogc.wfs.v1_1_0.catalog.common.GetCapabilitiesRequest;
import org.codice.ddf.spatial.ogc.wfs.v1_1_0.catalog.common.Wfs;
import org.codice.ddf.spatial.ogc.wfs.v1_1_0.catalog.common.Wfs11Constants;
import org.codice.ddf.spatial.ogc.wfs.v1_1_0.catalog.converter.FeatureConverterFactory;
import org.codice.ddf.spatial.ogc.wfs.v1_1_0.catalog.source.reader.FeatureCollectionMessageBodyReaderWfs11;
import org.codice.ddf.spatial.ogc.wfs.v1_1_0.catalog.source.reader.XmlSchemaMessageBodyReaderWfs11;
...
public class WfsSource extends MaskableImpl
  ...
  private void buildFeatureFilters(List<FeatureTypeType> featureTypes, List<String> supportedGeo)
    ...
    for (FeatureTypeType featureTypeType : featureTypes) {
      ...
      try {
        ...
        if ((schema != null)) {
          ...
          mcTypeRegs.put(
              ftName,
              new MetacardTypeRegistration(ftMetacard, props, featureTypeType.getDefaultSRS()));
          ...
          if (!CollectionUtils.isEmpty(featureConverterFactories)) {
            ...
            if (featureConverter == null) {
              LOGGER.debug(
                  ...
                  getId(),
                  ftName);
              featureConverter = new GenericFeatureConverter(featureTypeType.getDefaultSRS());
            }
          } else {
            LOGGER.debug(
                "WfsSource {}: Unable to find a feature specific converter; {} will be converted using the GenericFeatureConverter",
    ...
    if (!mcTypeRegs.isEmpty()) {
      ...
      for (Entry<String, MetacardTypeRegistration> entry : mcTypeRegs.entrySet()) {
  ...
  public SourceResponse query(QueryRequest request) throws UnsupportedQueryException {
    ...
    JAXBEqualsStrategy strategy = JAXBEqualsStrategy.INSTANCE;
